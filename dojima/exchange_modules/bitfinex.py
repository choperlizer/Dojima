...
PRETTY_NAME = "Bitfinex"
HOSTNAME = "api.bitfinex.com"
URL_BASE = "https://" + HOSTNAME + "/v1"
PLAIN_NAME = "bitfinex"
...
class _BitfinexPublicRequest(dojima.network.ExchangeGETRequest):
...
class BitfinexSymbolsRequest(dojima.network.ExchangeGETRequest):
    url = QtCore.QUrl(URL_BASE + "/symbols")

    def __init__(self, parent):
        self.parent = parent
        self.reply = None
        parent.requests.append( (1, self,) )
        parent.host_queue.enqueue(self.parent, 1)
    ...
    def _handle_reply(self, raw):
        ...
        for market_symbol in symbols:
            remote_base_id    = market_symbol[:3]
            remote_counter_id = market_symbol[3:]
            local_counter_id = dojima.model.commodities.remote_model.getRemoteToLocalMap('bitfinex-' + remote_counter_id)
            local_base_id = dojima.model.commodities.remote_model.getRemoteToLocalMap('bitfinex-' + remote_base_id)
            if ((local_base_id is None) or
                (local_counter_id is None)) : continue

            local_pair = local_base_id + '_' + local_counter_id

            if local_pair in self.parent.local_market_map:
                local_map = self.parent.local_market_map[local_pair]
            else:
                local_map = list()
                self.parent.local_market_map[local_pair] = local_map

            if market_symbol not in local_map:
                local_map.append(market_symbol)

            self.parent.remote_market_map[market_symbol] = local_pair

            dojima.markets.container.addExchange(self.parent, local_pair, local_base_id, local_counter_id)
        

class BitfinexExchangeProxy(dojima.exchange.ExchangeProxy):
    ...
    symbolsChanged = QtCore.pyqtSignal(str)

    def __init__(self):
        self.exchange_object = None
        self.network_manager = dojima.network.get_network_manager()
    ...
    def getExchangeObject(self):
        if self.exchange_object is None:
            self.exchange_object = BitfinexExchange()
    ...
    def getPrettyMarketName(self, remote_market_id):
        ...
        if len(s) is 6:
            s = remote_market_id[:3] + '/' +  remote_market_id[3:]
            return s.upper()

        return remote_market_id.upper()

    def getWizardPage(self, wizard):
        return BitfinexWizardPage(wizard)
    ...
    def refreshMarkets(self):
        BitfinexSymbolsRequest(self)
        
class BitfinexWizardPage(dojima.ui.wizard.ExchangeWizardPage):
    name = PRETTY_NAME
    ...
    def validatePage(self):
        ...
        base_symbol    = 'bitfinex-' + base_symbol
        counter_symbol = 'bitfinex-' + counter_symbol
...
class BitfinexExchange(QtCore.QObject, dojima.exchange.Exchange):
    valueType = Decimal
    ...
    def __init__(self, network_manager=None, parent=None):
        ...
        super(BitfinexExchange, self).__init__(parent)
    ...
    def cancelOffer(self, order_id, pair=None):
        ...
        request = BitfinexCancelOrderRequest(params, self)
    ...
    def placeAskLimitOffer(self, amount, price, pair):
        params = {'pair': pair,
                  ...
                  'rate': price,
                  'amount': amount}       
        request = BitfinexTradeRequest(params, self)
        request.pair = pair
    ...
    def populateMenuBar(self, menu_bar, market_id):
        ...
        edit_credentials_action = BitfinexEditCredentialsAction(account_menu)
    ...
    def refreshBalance(self, market_id=None):
        BitfinexInfoRequest(None, self)
                
    def refreshDepth(self, market_id):
        BitfinexDepthRequest(market_id, self)
    ...
    def refreshOffers(self, pair=None):
        ...
        BitfinexOrdersRequest(params, self)

    def refreshTrades(self, market_id):
        BitfinexTradesRequest(market_id, self)
        
    def _refresh_tickers(self):
        for pair in list(self.ticker_clients.keys()):
            BitfinexTickerRequest(pair, self)



class BitfinexDepthRequest(_BitfinexPublicRequest):
    path = "/depth"
...
class BitfinexTickerRequest(_BitfinexPublicRequest):
    path = "/ticker"
...
class BitfinexTradesRequest(_BitfinexPublicRequest):
    path = "/trades"
...
class _BitfinexPrivateRequest(dojima.network.ExchangePOSTRequest):
    priority = 1
    host_priority = 0
...
class BitfinexInfoRequest(_BitfinexPrivateRequest):
    method = 'getInfo'
...
class BitfinexCancelOrderRequest(_BitfinexPrivateRequest):
    method = 'CancelOrder'
    priority = 0
...
class BitfinexOrdersRequest(_BitfinexPrivateRequest):
    method = 'OrderList'
...
class BitfinexTradeRequest(_BitfinexPrivateRequest):
    method = 'Trade'
...
class BitfinexEditCredentialsAction(dojima.exchange.EditCredentialsAction):

    def show_dialog(self):
        dialog = BitfinexEditCredentialsDialog(self.parent())
        if dialog.exec_():
            self.accountSettingsChanged.emit()

        
class BitfinexEditCredentialsDialog(QtGui.QDialog):

    def __init__(self, parent=None):
        super(BitfinexEditCredentialsDialog, self).__init__(parent)
...
def parse_markets():
    ...
    exchange_proxy = BitfinexExchangeProxy()
